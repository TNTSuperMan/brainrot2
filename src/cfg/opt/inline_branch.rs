    ...
    fn internal_inline_branch(&mut self, block_i: usize) {
        ...
        let (pointer, zero, nonzero) = match self.0[block_i].edge {
            ...
            | CFGEdge::BranchWithIRAt {
                ...
                nonzero,
                ir_at: _,
            } => (pointer + self.0[block_i].offset.unwrap_or(0), zero, nonzero),
            CFGEdge::FindZeroAndJump { .. } => return,
            CFGEdge::End => return,
        };

        let last_assign = self.0[block_i]
            .insts
            .iter()
            .rev()
        ...
        if let Some(last_assign) = last_assign {
            if let CFGOp::Assign(_, CFGExpr::Value(CFGValue::Const(val))) = last_assign {
                self.update_edge(
                    block_i,
                    CFGEdge::Jump(if *val == 0 { zero } else { nonzero }),
                );
            }
            return;
        }
        match self.get_cellstate(block_i, pointer) {
            ...
            CellState::Const(_) | CellState::NonZero => {
                self.update_edge(block_i, CFGEdge::Jump(nonzero))
            }
            _ => {}
        }
    }
