use crate::cfg::{
    cfg::{CFG, CFGExpr, CFGOp, CFGValue},
    opt::cellstate::CellState,
};

impl CFG {
    fn internal_get_cellstate_inblock(
        &self,
        block_i: usize,
        inst_i: usize,
        pointer: i16,
    ) -> CellState {
        let last_assign = self.0[block_i].insts[0..inst_i]
            .iter()
            .rev()
        ...
        for i in 0..self.0[block_i].insts.len() {
            ...
            if let CFGOp::Assign(pointer, expr) = &mut self.0[block_i].insts[i] {
                ...
                match expr.clone() {
                    ...
                    CFGExpr::MulAdd(v1, CFGValue::Const(0), _) | CFGExpr::MulAdd(v1, _, 0) => {
                    ...
                    | CFGExpr::Mul(CFGValue::Load(p2), CFGValue::Const(c1)) => match c1 {
