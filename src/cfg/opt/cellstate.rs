...
impl CFG {
    pub fn internal_get_cellstate(
        ...
        let fallback = if let CFGEdge::Branch {
            pointer: b_pointer,
            zero,
            ..
        }
        ...
        } else if let CFGEdge::FindZeroAndJump {
            pointer: b_pointer,
            jumpto,
            ..
        } = &block.edge
        ...
        let last_assign = block
            .insts
            .iter()
            .rev()
            .find(|&inst| inst.writes() == Some(pointer));
    ...
    fn internal_cellstate_recurse(
        ...
        match self.0[block_i].predecessor.as_slice() {
            [] => CellState::Const(0),
            [pred] => self.internal_get_cellstate(block_i, *pred, pointer, recursive_count + 1),
            preds => {
                let (first, preds) = preds.split_first().unwrap();
                let state =
                    self.internal_get_cellstate(block_i, *first, pointer, recursive_count + 1);
                if preds.iter().all(|p| {
                    state == self.internal_get_cellstate(block_i, *p, pointer, recursive_count + 1)
                }) {
                    state
                } else {
                    CellState::Unknown
